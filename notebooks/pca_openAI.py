...
for string in strings:
    response = client.embeddings.create(input=string, model="text-embedding-3-large")
    prompt_embedding = response.data[0].embedding
    vectors.append(prompt_embedding)
...
response = client.embeddings.create(input=dart, model="text-embedding-3-large")
dart_embedding = response.data[0].embedding
dart_2d = pca.transform([dart_embedding])
dart_2d = dart_2d[0]
